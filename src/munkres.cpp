...
unsigned int
minsize(cv::Mat_<double> &m) {
    return (m.rows < m.cols) ? m.rows : m.cols;
}

double  maxValue(cv::Mat_<double> &m) 
{

    double max = 0;
    for (int i = 0; i < m.rows; i++) {
        for (int j = 0; j < m.cols; j++) {
            max = std::max<double>(max, m(i,j));
        }
    }
    return max;
}

void    extendMat(cv::Mat_<double> &m, unsigned int rows, unsigned int cols, double value = 0.0) 
{
    cv::Size2i interSize;
...
void    replace_infinites(cv::Mat_<double> &matrix) 
{
...
void
minimize_along_direction(cv::Mat_<double> &matrix, bool over_columns) {
...
int Munkres::step3(void) {
    ...
    const unsigned int ncols = static_cast<unsigned int>(matrix.cols);
    for ( unsigned int ncol = 0 ; ncol < ncols; ncol++ ) {
...
Munkres::step4(void) {
    ...
    const unsigned int nrows = static_cast<unsigned int>(mask_matrix.rows);
    const unsigned int ncols = static_cast<unsigned int>(mask_matrix.cols);
 ...
 */
void
Munkres::solve(cv::Mat_<double> &m) {
